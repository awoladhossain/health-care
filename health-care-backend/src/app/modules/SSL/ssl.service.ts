...
const initPayment = async (paymentData: any) => {
  try {
    ...
    throw new ApiError(500, "Internal server error from SSL");
  }
};
...
const validatePayment = async (payload: any) => {
  try {
    const response = await axios({
      method: "GET",
      url: `${config.ssl.ssl_validation_api}?val_id=${payload.val_id}&sotre_id=${config.ssl.storeId}&store_passwd=${config.ssl.storePasswd}&format=json`,
    });
    return response.data;
  } catch (error) {
    throw new ApiError(500, "Internal server error from SSL");
